...
template < typename Key, typename Value >
  class Node {
    public: Node(const Key & key,
      const Value & value, Node < Key, Value > * parent);
    virtual~Node();

    const std::pair <
      const Key, Value > & getItem() const;
    std::pair <
    const Key, Value > & getItem();
...
template < typename Key, typename Value >
  const std::pair <
    const Key, Value > & Node < Key, Value > ::getItem() const {
      return item_;
    }
...
template < typename Key, typename Value >
  std::pair <
  const Key, Value > & Node < Key, Value > ::getItem() {
    return item_;
  }
...
template < typename Key, typename Value >
  class BinarySearchTree {
    public: BinarySearchTree(); //TODO
    virtual~BinarySearchTree(); //TODO
    virtual void insert(const std::pair <
      const Key, Value > & keyValuePair); //TODO
      ...
      class iterator // TODO
    {
      ...
      std::pair <
      const Key, Value > & operator * () const;
      std::pair <
      const Key, Value > * operator -> () const;
      ...
      protected: friend class BinarySearchTree < Key,
      Value > ;
...
template < class Key, class Value >
  Value 
const & BinarySearchTree < Key, Value > ::operator[](const Key & key) const {
  Node < Key, Value > * curr = internalFind(key);
  if (curr == NULL) throw std::out_of_range("Invalid key");
  return curr -> getValue();
}
...
template < class Key, class Value >
  ...
  BinarySearchTree < Key, Value > ::predecessor(Node < Key, Value > * current) {
    ...
    if (current == NULL) {
      ...
      while (temp != NULL && current == temp -> getLeft()) /*runs until at root meaning parent is null and checking that if current node is not left child then exit and the parent is the predecessor*/ {
        current = temp; //must move current
        temp = temp -> getParent(); // moving temp
      }
      return temp;
    }
  }
...
template < typename Key, typename Value >
  Node < Key, Value > * BinarySearchTree < Key, Value > ::internalFind(const Key & key) const {
    ...
    return nullptr; //sanity check to make sure that if you hit there, it is a definite value that we are aware of, safety check
  }
...
template < class Key, class Value >
  Node < Key, Value > * BinarySearchTree < Key, Value > ::successor(Node < Key, Value > * current) {
    if (current == NULL) {
      ...
      Node < Key, Value > * temp = current -> getParent();
      while (temp != NULL && current == temp -> getRight()) //traversing up ancestor chain and checking if at root and always checking that we are in right subtree
      {
        current = temp;
        temp = temp -> getParent();
      }
